 ...
 * `auto_switch_add` : Default `false`. You can specify this option as a function where you can add `swtich` element into specified node. This function will pass node element ( jQuery object ) as a first argument.
...
//Dynamic switch
$('#tree').tree({
  'auto_switch_add': function(node){ return node.prepend($('<i>').addClass('switch icon-plus')); },
});
